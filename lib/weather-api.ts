...
export async function getWeatherByCoordinates(
  ...
  const params = new URLSearchParams({
    ...
    timezone: 'auto',
    // today + the 5 forecast days the dashboard renders
    forecast_days: '6'
  });
...
export async function getWeather(cityName: string, signal?: AbortSignal): Promise<{
  weather: WeatherResponse;
  location: GeocodingResult;
}> {
  const cities = await searchCity(cityName, signal);
...
const WEATHER_INTERPRETATIONS: Record<number, WeatherInterpretation> = {
...
export function interpretWeatherCode(code: number): WeatherInterpretation {
  return WEATHER_INTERPRETATIONS[code] || { description: 'Unknown', icon: '❓' };
}
